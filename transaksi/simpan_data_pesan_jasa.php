...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($result_petugas->num_rows > 0 && $result_pelanggan->num_rows > 0 && $result_layanan->num_rows > 0 && $result_kain->num_rows > 0) {
        ...
        // Memastikan tidak ada entri duplikat di tabel pesan_jasa
        $sql_check = "SELECT COUNT(*) as count FROM pesan_jasa WHERE id_petugas = ? AND id_pelanggan = ? AND id_layanan = ? AND id_kain = ? AND tanggal_pesan = ? AND tanggal_ambil = ? AND jumlah = ? AND status_pesan = ?";
        $stmt_check = $db->prepare($sql_check);
        if ($stmt_check === false) {
            echo json_encode(array('status' => 'error', 'message' => 'Prepare statement gagal: ' . $db->error));
            exit;
        }
        $stmt_check->bind_param('iiiissis', $id_petugas, $id_pelanggan, $id_layanan, $id_kain, $tanggal_pesan, $tanggal_ambil, $jumlah, $status_pesan);
        $stmt_check->execute();
        $result_check = $stmt_check->get_result();
        $check_data = $result_check->fetch_assoc();

        if ($check_data['count'] > 0) {
            echo json_encode(array('status' => 'error', 'message' => 'Data sudah ada di tabel pesan_jasa'));
            exit;
        }

        // Simpan data ke tabel pesan_jasa
        $sql_insert = "INSERT INTO pesan_jasa (id_petugas, id_pelanggan, id_layanan, id_kain, tanggal_pesan, tanggal_ambil, jumlah, status_pesan) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
